...
(function (g) {
  ...
  function create(opts) {
    ...
    var undoOps = [], redoOps = [];        // 스타일 op 데이터 (LIFO)
    var editLog = [], redoLog = [];        // {kind:'style'|'native'} 순서 로그
    var lastNativeT = 0, lastNativeType = '';
    function applyItems(items) {           // 적용하며 이전 값 반환(역방향 기록)
      return items.map(function (it) {
        var cur = {};
        for (var k in it.props) {
          cur[k] = it.el.style[k] || '';
          it.el.style[k] = it.props[k];
        }
        return { el: it.el, props: cur };
      });
    }
    function setStylesUndoable(items) {    // items: [{el, props:{prop:새값}}]
      undoOps.push({ items: applyItems(items) });
    ...
    function onUndoKey(e) {
      ...
      if (isUndo) {
        ...
        if (top.kind === 'style') {
          ...
          editLog.pop();
          if (op && op.items.every(function (it) { return doc.contains(it.el); })) {
            e.preventDefault(); e.stopPropagation();
            undoOps.pop();
            redoOps.push({ items: applyItems(op.items) });
        ...
        if (rtop.kind === 'style') {
          ...
          redoLog.pop();
          if (r && r.items.every(function (it) { return doc.contains(it.el); })) {
            e.preventDefault(); e.stopPropagation();
            redoOps.pop();
            undoOps.push({ items: applyItems(r.items) });
            editLog.push({ kind: 'style' });
            onChange(); syncLH(); syncSize();
          }
        } else {
