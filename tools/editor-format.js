...
(function (g) {
  ...
  function create(opts) {
    ...
    function hidePlus() { if (plusBtn) plusBtn.style.display = 'none'; plusTarget = null; }
    function onMove(e) {
      if (!enabled) return;
      if (plusRaf) return;
      plusRaf = win.requestAnimationFrame(function () {
        ...
        if (t.closest('.__fmt_plus')) return;                      // 버튼 위 → 유지
        if (t.closest('.__eui, .__he')) { hidePlus(); return; }
        var blk = paraOf(t);
        if (!blk) { hidePlus(); return; }
