...
(function (g) {
  ...
  function create(opts) {
    ...
    function ensureStyle() {
      ...
      s.textContent =
        '.__fmt_bar{position:fixed;z-index:2147483300;display:flex;align-items:center;gap:5px;flex-wrap:wrap;max-width:min(96vw,640px);' +
          'background:#16213a;color:#fff;padding:7px 10px;border-radius:12px;box-shadow:0 10px 32px rgba(0,0,0,.4);font-family:system-ui,"Malgun Gothic",sans-serif;font-size:12.5px;}' +
    ...
    function applyLineHeight(pct) {
      ...
      blocks.forEach(function (b) { b.style.lineHeight = pct + '%'; });
      onChange();
    }
    function syncLH() {
      if (!bar) return;
    ...
    function applyFontSizePx(px) {
      ...
      if (sel.isCollapsed) {
        ...
        if (blk) { blk.style.fontSize = px + 'px'; onChange(); }
        return;
      }
      try { doc.execCommand('styleWithCSS', false, true); } catch (e) {}
    ...
    function buildBar() {
      ...
      b.innerHTML =
        ...
        '<span class="sep"></span>' +
        '<button type="button" data-a="table" title="표">▦ 표</button>' +
        '<button type="button" data-a="callout" title="콜아웃">▍콜아웃</button>' +
        (onMedia ? '<button type="button" data-a="media" title="미디어 삽입">➕ 미디어</button>' : '') +
        '<span class="sep"></span>' +
        '<button type="button" data-a="clear" title="서식 지우기">지우기</button>';
      ...
      b.querySelector('.__fmt_blk').addEventListener('change', function () {
        ...
        if (v === 'note') {
          exec('formatBlock', 'p');
          var el = currentBlock();
          if (el) { el.style.color = '#8a94a3'; el.style.fontSize = '13px'; onChange(); }
        } else {
    ...
    function onSelChange() {
      ...
      selTimer = setTimeout(function () {
        ...
        keepRange();
        // 캐럿이 블록 밖(래퍼 div 등)이어도, 드래그 선택이 블록들을 걸치면 바를 띄운다
        if (!currentBlock() && (sel.isCollapsed || !blocksInRange().length)) { hideBar(); return; }
        showBar();
      }, 140);
    }
    ...
    function onScroll() { if (bar && bar.style.display !== 'none') positionBar(); }
