...
(function (g) {
  ...
  function create(opts) {
    ...
    // ---------------------------------------------------------------- 코드 서식
    // 문서에 code/pre 스타일이 이미 있으면 존중, 없으면 기본 스타일 1회 주입(저장본 유지).
    function ensureCodeStyle() {
      if (doc.getElementById('__code_css')) return;
      var probe = doc.createElement('code');
      probe.textContent = 'x';
      probe.style.position = 'absolute'; probe.style.left = '-9999px';
      doc.body.appendChild(probe);
      var cs = win.getComputedStyle(probe);
      var styled = cs.backgroundColor !== 'rgba(0, 0, 0, 0)' && cs.backgroundColor !== 'transparent';
      probe.remove();
      if (styled) return;   // 문서 자체 code 스타일 존재
      var s = doc.createElement('style');
      s.id = '__code_css';
      s.textContent =
        'code{background:#f1f5f9;border-radius:4px;padding:.1em .35em;font-family:Consolas,"D2Coding",monospace;font-size:.88em;}' +
        'pre{background:#f1f5f9;border:1px solid #e5e7eb;border-radius:8px;padding:1rem;overflow-x:auto;font-size:.85rem;font-family:Consolas,"D2Coding",monospace;}' +
    ...
    function toggleInlineCode() {
      ...
      var codeEl = ae && ae.closest('code');
      var fCode = fe && fe.closest('code');
      if (codeEl && codeEl === fCode) {
        // 이미 코드 → 해제 (자체 undo)
        var parent = codeEl.parentNode, next = codeEl.nextSibling;
        var kids = [];
        while (codeEl.firstChild) { kids.push(codeEl.firstChild); parent.insertBefore(codeEl.firstChild, codeEl); }
        codeEl.remove();
        pushOp({
          undo: function () { kids.forEach(function (k) { codeEl.appendChild(k); }); parent.insertBefore(codeEl, next); },
          redo: function () { while (codeEl.firstChild) parent.insertBefore(codeEl.firstChild, codeEl); codeEl.remove(); },
          alive: function () { return doc.contains(parent); }
        });
        return;
      }
      if (sel.isCollapsed || !String(sel).trim()) { toast('코드로 만들 텍스트를 선택하세요.'); return; }
      ensureCodeStyle();
      var txt = String(sel);
      try { doc.execCommand('insertHTML', false, '<code>' + esc(txt) + '</code>'); } catch (e) {}
      onChange();
    }
    ...
    function buildBar() {
      ...
      b.innerHTML =
        '<div class="frow">' +
        '<select class="__fmt_blk"><option value="">문단</option><option value="h4">제목</option><option value="p">본문</option><option value="note">주석</option><option value="pre">코드 블록</option></select>' +
        '<select class="__fmt_size">' + sizeOpts + '</select>' +
        '<button type="button" data-a="bold"><b>B</b></button>' +
        '<button type="button" data-a="code" title="인라인 코드 (선택 텍스트를 코드로/해제)">&lt;/&gt;</button>' +
      ...
      b.querySelector('.__fmt_blk').addEventListener('change', function () {
        ...
        if (v === 'note') {
          ...
          if (v === 'pre') {
            ensureCodeStyle();
            // 문단 "일부"만 선택된 상태면 통째 변환 대신 선택 영역만 인라인 코드로
            var s2 = restoreRange();
            var blk2 = currentBlock();
            var selTxt = s2 && !s2.isCollapsed ? String(s2).trim() : '';
            if (selTxt && blk2 && selTxt !== blk2.textContent.trim()) {
              toggleInlineCode();
              return;
            }
          }
